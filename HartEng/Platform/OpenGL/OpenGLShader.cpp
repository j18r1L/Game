...
namespace HE
{
	...
	std::shared_ptr<OpenGLShader> OpenGLShader::CreateFromString(const std::string& source)
	{
		std::shared_ptr<OpenGLShader> shader = std::shared_ptr<OpenGLShader>(new OpenGLShader());
		shader->Load(source);
		return shader;
	}
	...
	void OpenGLShader::AddShaderReloadedCallback(const std::function<void()>& callback)
	{ 
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			HE_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel" || type == "compute", "Invalid shader type specified");
	...
	int32_t OpenGLShader::GetUniformLocation(const std::string& name) const
	{
		...
		if (result == -1)
			HE_CORE_WARN("Could not find uniform '{0}' in shader", name);
	...
	void OpenGLShader::UploadUniformFloat(const std::string& name, float value)
	{
		...
		if (location != -1)
			...
			HE_CORE_ERROR("Uniform '{0}' not found!", name);
	}
